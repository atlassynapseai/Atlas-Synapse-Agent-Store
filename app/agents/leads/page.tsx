...
export default function LeadsPage() {
  ...
  async function analyze() {
    if (!input.trim()) {
      setError('Please paste a prospect profile or lead summary before running the agent.')
      return
    }
    ...
    setError(null)
    try {
      const res = await fetch('/agents/api/score-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ input }),
      })
      const data: LeadReport = await res.json()
      if (!res.ok || data.error) setError(data.error ?? 'Scoring failed. Please try again.')
      else setReport(data)
    } catch {
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        ...
        {report && tier && (
          <div className="space-y-4">
            ...
            <p className="text-white/20 text-xs text-center pb-4">Analysis generated by Atlas Synapse Lead Scoring Agent</p>
          </div>
        )}

        <AgentRunHistory agentName="Lead Scoring Agent" />
      </div>
    </div>
  )
}
