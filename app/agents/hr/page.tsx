...
export default function HRPage() {
  ...
  async function analyze() {
    if (!input.trim()) {
      setError('Please paste the job description and candidate profiles before running the agent.')
      return
    }
    ...
    setError(null)
    try {
      const res = await fetch('/agents/api/screen-candidate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ input }),
      })
      const data: HRReport = await res.json()
      if (!res.ok || data.error) setError(data.error ?? 'Screening failed. Please try again.')
      else setReport(data)
    } catch {
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        ...
        {report && (
          <div className="space-y-4">
            ...
            <p className="text-white/20 text-xs text-center pb-4">Analysis generated by Atlas Synapse Recruitment Screening Agent</p>
          </div>
        )}
        <AgentRunHistory agentName="HR Screening Agent" />
      </div>
    </div>
  )
}
