...
export default function CompliancePage() {
  ...
  async function analyze() {
    if (!input.trim()) {
      setError('Please paste a policy or procedure document before running the agent.')
      return
    }
    ...
    setError(null)
    try {
      const res = await fetch('/agents/api/check-compliance', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ input }),
      })
      const data: ComplianceReport = await res.json()
      if (!res.ok || data.error) setError(data.error ?? 'Assessment failed. Please try again.')
      else setReport(data)
    } catch {
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        ...
        {report && risk && (
          <div className="space-y-4">
            ...
            <p className="text-white/20 text-xs text-center pb-4">Analysis generated by Atlas Synapse Regulatory Compliance Agent</p>
          </div>
        )}
        <AgentRunHistory agentName="Regulatory Compliance Agent" />
      </div>
    </div>
  )
}
