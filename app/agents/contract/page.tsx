...
export default function ContractPage() {
  ...
  async function analyze() {
    if (!input.trim()) {
      setError('Please paste a contract or legal document before running the agent.')
      return
    }
    ...
    setError(null)
    try {
      const res = await fetch('/agents/api/analyze-contract', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ input }),
      })
      const data: ContractReport = await res.json()
      if (!res.ok || data.error) setError(data.error ?? 'Analysis failed. Please try again.')
      else setReport(data)
    } catch {
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        ...
        {report && (
          <div className="space-y-4">
            ...
            <p className="text-white/20 text-xs text-center pb-4">Analysis generated by Atlas Synapse Contract Analysis Agent</p>
          </div>
        )}
        <AgentRunHistory agentName="Contract Analysis Agent" />
      </div>
    </div>
  )
}
