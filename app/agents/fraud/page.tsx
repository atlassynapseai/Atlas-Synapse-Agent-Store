...
export default function FraudPage() {
  ...
  async function analyze() {
    if (!input.trim()) {
      setError('Please paste transaction data or a fraud incident before running the agent.')
      return
    }
    ...
    setError(null)
    try {
      const res = await fetch('/agents/api/detect-fraud', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ input }),
      })
      const data: FraudReport = await res.json()
      if (!res.ok || data.error) setError(data.error ?? 'Fraud analysis failed. Please try again.')
      else setReport(data)
    } catch {
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        ...
        {report && risk && (
          <div className="space-y-4">
            ...
            <p className="text-white/20 text-xs text-center pb-4">Analysis generated by Atlas Synapse Fraud Detection Agent</p>
          </div>
        )}
        <AgentRunHistory agentName="Fraud Detection Agent" />
      </div>
    </div>
  )
}
